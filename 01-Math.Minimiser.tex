...
% <Vector Exercise>
  \begin{frame}{Vectors}
    \begin{exampleblock}{Exercise}
      ...
      \begin{equation*}
        \vec{v} = \left(1, -2, 5, 0\right)
        \quad\mathrm{and}\quad
        \vec{w} = \left(2, 2, 3, -1\right)
      \end{equation*}
      \begin{enumerate}
        \item Calculate the lengths $|\vec{v}|$, $|\vec{w}|$
        \item Calculate the sum $\vec{v} + \vec{w}$
...
% <Convolution>
  \begin{frame}{Convolution}
    \begin{itemize}
      \item Mathematically
        \begin{equation*}
          \left(f * g\right)(t) \coloneqq \int f(\tau)g(t-\tau)\deriv{\tau}
        \end{equation*}
      \begin{itemize}
        \item Intuitive: $f$ is \enquote{signal}, $g$ is \enquote{response}.
      ...
      \begin{equation*}
        (f*g)[i] = \sum_{k=-N}^{N} f[k]g[i-k]
      \end{equation*}
      where $\{-N, -N+1, \dots, N-1, N\}$ is the support of $g$
      \begin{itemize}
