# coding: utf-8
"""
Example plots for a LDA classification
"""
...
lda_2 = LinearDiscriminantAnalysis(n_components=None)
lda.fit(data, classes)
lda_2.fit(data2, classes2)
...
preds = lda.predict_proba(grid_points)[:, 1]
preds_classes = lda.predict(grid_points)
preds_2 = lda_2.predict_proba(grid_points)[:, 1]
preds_classes_2 = lda_2.predict(grid_points)
...
# Contours
axb.contourf(XX, YY, preds_classes_2.reshape(XX.shape),
             levels=[0., 0.5, 1.], vmin=0, vmax=1, alpha=0.1, colors=colors)
axb.contour(XX, YY, preds_classes_2.reshape(XX.shape),
            levels=[0.5], linewidths=2, colors=["C7"])
...
fig.tight_layout()
plt.savefig("../build/assets/02-img-algos_lda.png", dpi=200)
# plt.show()
